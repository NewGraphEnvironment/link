...
- Phase 3 — `lnk_pipeline_mapping_code` shipped. **ADMS parity 15762/15762 byte-identical for all 8 species** (BT, CH, CM, CO, PK, SK, ST, WCT). `lnk_pipeline_access` extended with `barrier_sources` arg + auto-NA propagation when a source table has 0 rows in the AOI (mirrors bcfp's `IS NULL`-column semantics that yield empty mapping_code for absent species like ST/WCT in ADMS). Resident-flavor (BT/WCT) uses sequence-aware `dam_dnstr_ind` from bcfp's pre-computed table; computing that from primitives in link is Phase 5 polish (only matters in mixed-barrier-type WSGs, ADMS test merges bcfp's value for now).
- Two species behaviours that surprised: (1) CM/PK use the *anadromous* token2 flavor + spawn-only token1 (no rearing); (2) bcfp's NULL semantics on `barriers_<sp>_dnstr` for absent species (ST/WCT in ADMS) yield `""` not `ACCESS;NONE` — required NA propagation through `lnk_pipeline_access` to mapping_code rather than a presence-driven shortcut (which would've broken CM/PK for shared-table WSGs).
