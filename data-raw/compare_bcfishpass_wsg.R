...
compare_bcfishpass_wsg <- function(wsg, config) {
  ...
  # -------------------------------------------------------------------------
  # Bcfishpass-side rollup. Reads `bcfishpass.habitat_linear_<sp>` —
  # the rule-based boolean classification (one table per species).
  # This is apples-to-apples with link's bcfishpass-bundle output,
  # which reproduces bcfishpass's RULE classification (overlay
  # disabled in the bundle's pipeline.apply_habitat_overlay flag).
  # The published `streams_habitat_linear` adds bcfishpass's separate
  # known-habitat overlay; that's a post-classification step that the
  # default bundle exercises but the bcfishpass bundle does not.
  #
  ...
  ref_list <- lapply(species, function(sp) {
    ...
    # Linear km — when habitat_linear_<sp> doesn't exist for this
