...
n_cy <- length(cy_ws)
# R quirk: paste0("cy", integer(0)) returns "cy" (length 1) instead of
# character(0) — R recycles the constant when one arg is length 0. Without
# the explicit n_cy == 0L branch, --no-cyphers mode generates a phantom
# "cy" host_key that catches ~25% of WSGs in the LPT loop. Those WSGs
# never appear in M4= / M1= output lines and get silently dropped at the
# bash extraction step.
cy_host_keys <- if (n_cy == 0L) character(0)
                else if (n_cy == 1L) "cy"
                else paste0("cy", seq_len(n_cy))
