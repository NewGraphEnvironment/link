...
**The dispatcher's `fresh_sha` was never missing. It was unread.** The standing explanation, repeated in `CLAUDE.md`, `RUNBOOK.md`, `research/study_area_run.md` and `data-raw/study_area_verify.sql`, was that m1 installs `fresh` locally (`RemoteType: local`, no `RemoteSha`). Measured 2026-09-01: m1's install carries `RemoteType github`, `RemoteRef v0.33.0` and a `RemoteSha` byte-identical to the SHA every cypher records. `.lnk_pkg_git_sha()` read an env var, then walked for `.git`, then gave up — and an installed package has no `.git`. **The tolerance in the verifier was a workaround for an unread field, not an absent one**, and a false rationale invites the fix in the wrong direction.

**One resolver, two wrappers.** New `.lnk_pkg_git_state()` returns `list(sha, dirty, source)`; `.lnk_pkg_git_sha()` and `.lnk_pkg_git_dirty()` delegate. Two functions reading the same DESCRIPTION and the same checkout are free to drift about one package, silently. `sha` and `dirty` still walk their **own** tier lists over that shared lookup, because `cypher_prep.sh` has always written `FRESH_GIT_SHA` and never `FRESH_GIT_DIRTY` — a design where one env var claims the whole state leaves `dirty` NA on exactly the host that set the SHA.

**Two inferences that had to be bounded rather than taken.** Tier 2's `dirty = FALSE` is sound — `RemoteSha` is only written for an install from a published ref, which is not a working tree — but it is gated on `identical(sha, remote_sha)`, since applied to any other SHA it certifies a commit the host never built. And the tier applies a **hex shape guard**: a `RemoteType: standard` install writes the package *version* into `RemoteSha`, and **278 of 300** packages in this library carry a non-hex value there. Unfiltered it would put `fresh_sha = "0.33.0"` in a column two consumers read as a commit.

**`bcfishobs` is a model input, not a reference dataset.** `lnk_barrier_overrides()` counts its observations upstream of each barrier to decide which barriers are skipped, so different `bcfishobs` code gives different barriers, different access, a different `mapping_code`. New `bcfishobs_sha` column and `.lnk_bcfishobs_sha()`, sharing one body with `.lnk_fwapg_sha()`. **No `bcfishobs_dirty` column**, against the issue's own proposal: `fwapg` has none either, because `preflight_local()` refuses to run over a dirty checkout and a column `FALSE` on every row is #257 pointing the other way. New `fresh_sha_source` records which tier answered, mirroring `bcfp_pin_source`.

**`fresh_sha` is now a pre-flight parity key.** It was excluded alongside `link_sha` on the joint grounds that one can only ever fail and the other could only ever pass vacuously. Only the first half survives. It is the one field proving every host runs the same fresh **build** rather than the same version string — #246 exactly, where a cypher silently ran the image's fresh.

**Where the review paid.** Three rounds found fourteen defects, and rounds 2 and 3 found them **inside the previous round's fixes**. `scripts/update_hosts.sh` installs via `R CMD INSTALL` of a source tarball to route around r-lib/pak#658, which writes no `Remote*` fields — so the new assertions would have failed that host *after* droplets were paid for. Teaching it to record what it installed then introduced three fresh defects of its own: `| tail -3` swallowed a failed install so the pin was stamped for a build that never happened; pinning `LINK_GIT_DIRTY=false` would have made `link_dirty` permanently FALSE on the `load_all` dispatcher, which is #257 stuck in the direction nobody can notice; and resolving the SHA per host raced a mid-run push. The verifier gained a `fresh_dirty = TRUE` arm it had lacked entirely — the first draft gave the weaker `IS NULL` state a NOTE while the state that makes the newly-mandatory `fresh_sha` a *lie* closed the run with PASS.

**Verified against the database, not an exit code.** A log table built with the pre-#264 column set gains both columns via `ADD COLUMN`, and a real write lands all four values on `log` *and* `log_recompute`. The verifier was swept across nine single-fault states on a synthetic two-host fixture — every one answering correctly, healthy passing before *and* after, since a mutation left unrestored would leave later cases printing ticks against damaged data. The first attempt at that sweep reported a correct FAIL that had actually errored on a column the fixture lacked, which is why the post-restore case exists.

**Rows logged before this release keep their NULLs and will now fail the verifier.** That is correct: they cannot say which `fresh` build produced them, and it is not retroactively fixable.

Suite 1894 pass / 0 fail, 16 warnings (0.49.0 baseline: 1825 / 0 / 16).

# link 0.49.0

Closes the provenance gaps that cannot be fixed after a run ([#262](https://github.com/NewGraphEnvironment/link/issues/262), [#257](https://github.com/NewGraphEnvironment/link/issues/257)). The 217-WSG provincial run is next, and #246 exists because 93 WSGs once accumulated with no log tables at all — a partial record at 217 repeats that at scale.

**Two of the four reported gaps were wired and unfed, and a third named the wrong source.** `run_label` has been threaded from `lnk_pipeline_run()` to the INSERT since #127; it was NULL because nothing ever set `LNK_RUN_LABEL`. `.lnk_bcfp_log_current()` has been called at run open just as long; it returns NULL because it queries `bcfishpass.log` and the local docker fwapg holds **zero** `bcfishpass` tables — the run is tunnel-free by design. Implementing what the issue literally asked for would have added code beside working code twice and recorded a wrong value the third time.

**A dispatch is now one queryable unit.** New `run_uid` — minted once per `study_area_run.sh` invocation, shared by every host and every WSG — replaces "a time window plus a host list", which is fragile at 34 WSGs on three hosts and ambiguous the moment two runs overlap. `lnk_log_read()` gains `run_uid` (which overrides `latest`, since `DISTINCT ON` would hide a WSG re-run inside the same dispatch) and `phase`. The identifier reaches cyphers because it is exported on **both** legs — the local loop and the ssh command string; the local-only version is the `LNK_GUARD_DOWNSTREAM` trap of #227, and it fails silently rather than loudly.

**The recompute now records what it did.** `data-raw/wsg_recompute_one.R` rewrites `streams_access` and `streams_mapping_code` — the values that actually ship — and logged nothing, so `fresh.log` recorded when a WSG was *modelled* and said nothing about when its persisted access last *changed*. New `<schema>.log_recompute`, its own table rather than a `phase` column, because a recompute row in `log` would win `lnk_log_read()`'s `DISTINCT ON` and be returned as "what produced this network". It carries `watershed_group_code`, so `schema_consolidate.R` discovers it with no list to maintain, and `LNK_LOG=0` keeps benchmark passes (`recompute_sweep.sh`, `recompute_parity.sh`) out of it — otherwise the new verify check would pass on benchmark noise.

**`on.exit()` at the top level of an Rscript never fires**, which is how the first draft of that logging shipped a dead failure handler. It attaches to the global environment, which never exits — measured, on both `stop()` and `quit()`. The same file already carried two dead handlers for that reason, the likely source of the 49 orphaned `zz_lnk_mc_scratch_*` tables recorded in #246. The work now runs inside a function, where `on.exit` is real.

**The bcfp reference is pinned from the snapshot it was actually computed against.** Three tiers (`LNK_BCFP_MODEL_VERSION`, `bcfishpass.log`, then the local `bcfp_baselines.csv` ledger) with a new `bcfp_pin_source` recording which answered. The env tier is not redundant: the ledger is per host and a cypher has no row of its own, so without it the majority of a 217-WSG run lands unpinned — the same shape, and the same fix, as `FWAPG_GIT_SHA`. Querying a live `bcfishpass.log` at compare time, as the issue proposed, would name the build the tunnel is at *now* rather than the one the numbers came from. `bcfp_model_run_id` stays NULL on the tunnel-free path because `log.json` carries no such key; honest absence beats a fabricated id.

**`link_dirty` means something again (#257).** It was `t` on all 21 dispatcher rows of the first provenanced run while the tracked tree was byte-identical to origin — the run writes ~15 files into a *tracked* log directory, so it dirtied its own checkout by operating. The predicate now excludes `data-raw/logs` and nothing else: untracked files elsewhere still count, because a new uncommitted `R/*.R` is invisible to a cypher and that is exactly the drift being detected. `:(exclude)` in long form, since `:!` aborts and an aborted `git status` returns empty, which reads as clean. And `shQuote()`, because `system2()` shell-quotes the command but pastes arguments on raw — the parentheses in `:(top,exclude)` were parsed by the shell, the command never ran, and the predicate returned `NA` for every input. That one was caught by running it; it is invisible by reading.

**`data-raw/study_area_verify.sql` can now actually fail.** It keyed on a 6-hour window and a hardcoded 34-WSG `VALUES` list — a scope pinned to nothing, and wrong for 217 before it was attempted. It now takes `-v run_uid=` and asserts via `RAISE`, so the exit status means something. `expected_n` is supplied from **outside** on purpose: deriving the expected set entirely from `fresh.log` is circular, since a WSG that never produced a log row vanishes from "expected" and that is the failure being checked for. New `data-raw/study_area_verify_negative.sh` proves it fails when it should *and passes when it should* — the healthy case runs first, because a script that always exits non-zero "detects" everything.

# link 0.48.0

Runs the post-consolidate recompute N-wide ([#250](https://github.com/NewGraphEnvironment/link/issues/250)). It was a serial loop over every WSG in the run — **48 min of a 124 min run**, single-threaded on a 10-core host, with the cyphers already burned so it was pure dispatcher time. `--recompute-jobs=` (default 4, max 16) replaces it. #205 had already made the per-WSG cost cheap; what remained was entirely serialisation.

**The issue's own safety argument had a hole, and finding it is most of this release.** It states that each WSG's recompute reads shared tables and writes only its own rows, so loop order is irrelevant. True of everything except `lnk_access()`, which unconditionally rebuilt the **schema-scoped** barrier views — names every sibling WSG reads. That is two distinct failures: `DROP VIEW` and the following `CREATE` autocommit separately, leaving an interval in which the view does not exist; and `CREATE OR REPLACE VIEW` takes an `AccessExclusiveLock` whose *queued* request blocks every subsequent `AccessShareLock`, so one job's DDL stalls every sibling's read until `lock_timeout` kills someone — surfacing as a silent `[WARN]`. The fix is therefore both: hoist the build out of the loop (new `lnk_access(build_views =)`, new `data-raw/barriers_views_build.R`) and drop the redundant `DROP` (v0.47.3). `build_views = FALSE` **verifies** the views exist rather than trusting the caller, because an absent view otherwise fails deep inside the network walk with a bare `relation does not exist` that reads as one WSG's problem.

**Recompute failures could not previously fail the run at all.** `|| echo` sat *inside* the loop, so the subshell always exited 0 and the success line was unconditional; `RUN_INCOMPLETE` is assigned *before* the block and nothing after raised it. A run in which all 50 recomputes failed exited 0 and wrote a compare CSV. This is the more consequential half of the release — an incomplete recompute is *silently wrong* output, not missing output: those WSGs keep their pre-consolidate access, so token1/token2 and `;DAM` are wrong for them while the compare CSV looks complete. It now has its own message saying so, with the per-WSG re-run command, kept distinct from the bucket-incomplete message so an operator is not sent to the wrong host.

New exported **`lnk_fanout_judge()`** is what makes that possible. A parallel loop over an empty work list exits 0 exactly like a successful one, and a per-item loop reports the exit of whichever item ran last — neither is something shell answers reliably, and this repo has no shell test harness, so the predicate lives in R where testthat can prove it. Five statuses, and the distinctions are the point: `none_ran` is not `all_failed` (nothing recorded anything, which indicts the harness rather than the work), and a job that *failed* is not a job that *never reported* — calling the second a failure sends someone to read a log that does not exist. It takes expected job **names**, not a count, so it can say which are missing; `expected` has no default, following `n_expected` in `lnk_preflight_parity()`.

**The pool is hand-rolled rather than `xargs -P`.** `xargs` exits 123 for anything between 1 and 125, collapsing "1 of 50 failed" and "50 of 50 failed" into one status — exactly the distinction being reported. It tracks only *liveness* (`kill -0` plus `wait <pid>`), since `wait -n` is bash 4.3+ and `/bin/bash` on macOS is 3.2.57; exit status comes from a per-job `.rc` file written **last**, so a job killed partway is judged "never reported" rather than counted as fine, and the evidence survives a killed dispatcher. Per-job logs go in `${TS}_recompute.d/` rather than flat, because that log directory is **tracked** — flat files would add 217 committed files per provincial run to a public repo.

**Measured, and it is not the number the issue projected.** Speedup is **1.43x on 4 WSGs and 1.51x on 8** at `-j4`, not ~3.4x. The cause is neither the pool nor the database: a pool's makespan is bounded below by its slowest single job, and on both sets one WSG dominated -- CHWK at 226 s is 74% of the 4-WSG set and 52% of the 8-WSG set, so the ceilings were 1.53x and 1.92x and the pool reached 93% and 79% of them. Two confident diagnoses were wrong on the way there (a Postgres worker-pool theory, refuted by a `PGOPTIONS` sweep that made scaling *worse*; a Docker I/O theory, refuted by 269 `pg_stat_activity` samples that were 100% `CPU(running)`), and both came from theorising about the aggregate instead of reading the per-job distribution. Full record with the numbers in `research/recompute_parallel_2026_09_01.md`.

**Recompute cost does not track segment count** -- SALR is 20% larger than CHWK and finishes 20x faster, because the cost is the downstream barrier walk. So the pool now orders its work list **longest-first from prior recompute times** rather than alphabetically, falling back to input order (loudly) when no times exist. Note the recompute is *not* bucketed -- buckets distribute modelling across hosts, and that packing was already made speed-aware in [#253](https://github.com/NewGraphEnvironment/link/pull/253). Separately, checked against the 104 WSGs of modelling times already on disk, segment count predicts modelling time at R^2 0.636 with a 5x spread in unit cost; whether `comp_weight` should move to measured times is filed on its own, since measured times are not always present and a silently-degrading fallback is a recorded past incident.

Proof rather than assertion, since `lnk_access(merge = TRUE)` reads its own prior output (`WHEN t.access_<sp> = 2 THEN 2` preserves observed access) and so is *not* a pure function of untouched inputs. `data-raw/recompute_parity.sh` runs three passes from a snapshot — serial, serial again on top of it, then parallel and **shuffled** from the same pre-state — so a failure points at one thing: A≠B means the recompute is not idempotent (nothing to do with parallelism), A=B≠C means parallelism broke something. `data-raw/pool_probe.sh` exercises the shipped pool functions (extracted, not copied) under both bash 5.3.9 and 3.2.57.
...
Drops the `gr` (Arctic grayling) presence flag for **LARL** (Lower Arrow Lake) in the `default` config bundle ([#242](https://github.com/NewGraphEnvironment/link/pull/242)). Grayling is an Arctic-drainage species and LARL is Columbia; because `wsg_species_presence` gates whether a species is modelled at all, the flag was producing a full grayling habitat and mapping_code layer for the Lower Arrow Lake system. It was not fabricated — four LARL observations spanning 1954-1995, plus one each in BULL and ELKR — but six historical records across three Columbia groups, newest 1995, is a thin basis for modelling a species across a watershed group, and most are plausibly mid-century stocking that never established. Scoped to `default` **only**: bcfishpass carries `gr = true` for LARL upstream, so the bcfishpass bundle is left byte-identical to it rather than silently diverged, since that is the bundle used for parity. The two copies of this file were identical and now differ deliberately; they converge again if [NewGraphEnvironment/bcfishpass#14](https://github.com/NewGraphEnvironment/bcfishpass/issues/14) lands. No upstream PR was needed first because this file is hand-authored in link (`source: link (hand-authored)`, no `source: bcfp` in the `files:` block), so it is off the weekly csv-sync path. Also carries the 2026-08-12 csv-sync of `pscis_modelledcrossings_streams_xref.csv` ([#239](https://github.com/NewGraphEnvironment/link/pull/239)).

# link 0.45.1

Adopts `fresh` 0.33.0, which rebuilt `frs_wsg_drainage()` on per-group outlet points and the measure-aware `whse_basemapping.fwa_downstream()` ([fresh#214](https://github.com/NewGraphEnvironment/fresh/issues/214)). This closes the `public.wsg_outlet` half of [#227](https://github.com/NewGraphEnvironment/link/issues/227) by **removing the dependency** rather than repairing it — outlets now ship with fresh and reach the database as a `VALUES` list, so there is no ad-hoc table to materialize and none to lose on a docker volume rebuild. `test-lnk_wsg_resolve.R` closure-mode tests, red since the table went missing, pass again.

Closures tighten, so `lnk_wsg_resolve()` expectations move with them: `c("PARS", "BULK")` resolves to 9 WSGs where it previously returned 15. The dropped six were all over-inclusions of the old single-ltree predicate — LKEL is a separate Skeena tributary, MSKE and USKE sit upstream of the Bulkley confluence, LBTN is a Peace sibling, MORR is upstream of BULK, and FINA is a separate Williston arm the Parsnip never flows through. Study-area buckets built before this will differ from ones built after; the earlier runs were over-inclusive rather than short, so their results stand, but bucket membership is not comparable across the change. Adds a companion test pinning the exclusions by name, and bumps the `fresh` floor to 0.33.0.
# link 0.45.0

Run provenance on the persist schema ([#127](https://github.com/NewGraphEnvironment/link/issues/127)). A network in `<persist_schema>` previously carried no record of what produced it: given `streams` and `streams_mapping_code` you could not say which config was used, what that config actually said at the time, which vintage of the DB primitives it read, or when it was written. Four sidecar tables now answer that, mirroring `bcfishpass.log` and its `log_parameters_*` children — `log` (one row per `lnk_pipeline_run()`, opened before anything writes and closed on success), `log_parameters_fresh` and `log_dimensions` (the **full** CSV rows, so `observation_species = BT;DV` at threshold 1 with no date floor is recorded *in the database* rather than as a pointer to a file that will move on), and `log_input` (per-primitive fingerprints). Read them with the new `lnk_log_read()`. This is what makes scenario work tractable: [#236](https://github.com/NewGraphEnvironment/link/issues/236) runs the DV-as-BT override with and without, and without provenance both results land in the same schema indistinguishable a week later. Three design points earned their keep. `run_id` is text, not a serial, because `schema_consolidate.R` COPYs literal values between hosts whose sequences would both start at 1 — a serial guarantees PK collisions and silent loss of run history. `.lnk_log_align_columns()` issues `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` on every init, because `CREATE TABLE IF NOT EXISTS` silently no-ops on an existing table and therefore could never ship a *new* config column, which would reintroduce one layer up the exact silent drift the issue exists to prevent. And `config_hash` hashes the observed bytes of the **resolved** file set rather than the declared `provenance:` block, since `config.yaml` is absent from its own block — a declared-set hash would be blind to `pipeline$schema`, `break_order` and `gradient_classes`; `config_drift` is kept as the separate "did it match what it claimed" axis. Input provenance is read rather than recomputed, and is honest about its gaps: `bcdata.log` supplies authoritative download times but covers only `bc2pg` downloads, so **not FWA**, whose `pg_stat_user_tables` row is empty because it is bulk-restored — the stream network's only real version is the fwapg checkout commit, captured as `fwapg_sha`. There is deliberately no `count(*)` anywhere (exact counts on a 4.9M-row, 9.8 GB table across a provincial pass would add hours; row counts are `reltuples` estimates flagged as such). Everything after the open row degrades to a warning, so provenance logging can never kill a modelling run, and `on.exit` rather than `tryCatch` preserves `traceback()` and the `invisible(conn)` contract — leaving a three-state completion signal where `date_end` NULL with notes means an R error and NULL without means SIGKILL. `digest` moves from Suggests to Imports. Not yet included: `schema_consolidate.R` support for the two `config_hash`-keyed tables, so they do not yet travel between hosts.
