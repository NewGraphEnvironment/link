...
**Recompute cost does not track segment count** -- SALR is 20% larger than CHWK and finishes 20x faster, because the cost is the downstream barrier walk. So a bucketing scheme balanced on segments does not balance recompute time, and whether the 34-WSG field set contains a similar dominant item is unknown and decides the real speedup.
