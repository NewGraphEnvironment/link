# Crossing interpretation for Morice (MORR)
...
Provincial crossing assessments produce a binary classification: BARRIER
or PASSABLE. This tells you nothing about how bad a barrier is or what
habitat it blocks.
...
(passable). These come from systematic review by multiple people.
...
length, channel width. The crossings table has network position.
Matching links the measurements to the network so severity scoring can
use real data.
...
|----------|-----------------------------------------------|--------------------------|
| High     | outlet_drop \>= 0.6m OR slope\*length \>= 120 | Impassable at most flows |
| Moderate | outlet_drop \>= 0.3m OR slope\*length \>= 60  | Flow-dependent           |
...
produces the spec that plugs directly into `frs_habitat()`. Severity
labels translate to access labels via `label_map`.
