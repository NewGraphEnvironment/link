...
test_that("lnk_pipeline_connect rejects invalid inputs", {
  ...
  expect_error(
    lnk_pipeline_connect("mock", aoi = NULL, cfg = cfg, schema = "w"),
  ...
  expect_error(
    lnk_pipeline_connect("mock", aoi = "BULK", cfg = cfg, schema = "w",
...
test_that("lnk_pipeline_connect errors when species cannot be resolved", {
  cfg_stub <- structure(list(
    ...
    rules_yaml = system.file("extdata", "configs", "bcfishpass",
