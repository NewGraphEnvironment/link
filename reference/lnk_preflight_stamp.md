...
installed package. On the dispatcher link is
...
from a local source tree, so it does not and returns `NA`. Comparing
`link_sha` across hosts would therefore always fail — a check that looks
like a check. `repo_sha` is read from `~/Projects/repo/link` **on the
host itself**, which on a cypher is exactly what
...
comparison a `NA == NA` pass. That was measured false on 2026-09-01: the
dispatcher's installed `fresh` carried `RemoteSha 7f12d99115b7…`,
byte-identical to the value its cyphers recorded, and the resolver
simply never read the field. It does now, so `fresh_sha` resolves
everywhere and
