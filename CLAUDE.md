...
Not at parity with bcfishpass despite earlier "exact reproduction" framing. The bcfishpass-config bundle is missing barrier classes that bcfishpass uses: subsurface-flow (edge_type 1410/1425) is not modelled at all; falls are sourced from a static CSV in `fresh` rather than `cabd.waterfalls` + the four CABD edit CSVs (additions / exclusions / blkey_xref / passability_status_updates); dams are not modelled at all. Per-WSG diff_pct in the rollup reflects these gaps, not just methodology choice. Closing the gaps is the active work — issue #N (TBD) tracks subsurfaceflow + CABD wiring.
