...
[bcfishpass](https://github.com/smnorris/bcfishpass) produces. Second, it shows
`link` **extending** that methodology to a species bcfishpass does not model in
the Peace: Arctic grayling (`GR`).
...
does not — here, Arctic grayling for the **Fish Passage Peace 2025** program —
...
draw thickest, rearing medium, access-only thinnest. Both the colours and the
widths are read straight from the bcfishpass symbology registry bundled in
`gq`, so the maps match a bcfishpass QGIS project exactly.
...
draw_context_full()
plot(sf::st_geometry(bt), col = bt$col, lwd = bt$w * 0.6, add = TRUE)
...
draw_context_full()
plot(sf::st_geometry(gr), col = gr$col, lwd = gr$w * 0.6, add = TRUE)
plot(sf::st_geometry(aoi), col = NA, border = "black", lwd = 1.5, add = TRUE)
label_reserves(reserves)
...
panel <- function(seg, title) {
  ...
  if (!is.null(seg) && nrow(seg) > 0L) {
    plot(sf::st_geometry(seg), col = seg$col, lwd = seg$w, add = TRUE)
  }
  if (!is.null(aoi_d)) {
