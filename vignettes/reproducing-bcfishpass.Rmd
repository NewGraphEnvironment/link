...
.pivot <- function(rollup, which_habitat) {
  ...
  cols <- intersect(c("species", "ADMS", "BULK", "BABL", "ELKR"),
    names(w))
...
The tables cover ADMS / BULK / BABL / ELKR — the four parity
watershed groups. DEAD (Deadman River) is also run by `tar_make()`
but is reported separately in
[`research/bcfishpass_comparison.md`](https://github.com/NewGraphEnvironment/link/blob/main/research/bcfishpass_comparison.md);
it was added as a targeted end-to-end test for the
`barriers_definite_control` filter rather than a parity measurement.
